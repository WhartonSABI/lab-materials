...
```{r}
#try with equal probs for everything
a_vals <- seq(0.0, 0.5, length.out = 5)
e_vals <- seq(0.0, 1.0, length.out = 5)
b_vals <- seq(0.5, 1.0, length.out = 5)
c_vals <- seq(0.5, 1.0, length.out = 5)
d_vals <- seq(0.5, 1.0, length.out = 5)
...
ggplot(results3, aes(x = d, y = e, fill = pC)) +
  geom_tile() +
...
```{r}
n_points <- 5  # control resolution (adjust for speed vs granularity)
vals_01 <- seq(0.0, 1.0, length.out = n_points)
vals_05 <- seq(0.0, 1.0, length.out = n_points)
...
```{r}
ggplot(results4, aes(x = d, y = e, fill = pC)) +
  geom_tile() +
  scale_fill_gradientn(colours = pal)
  labs(
    title = "Equilibrium probability of shooting Center",
