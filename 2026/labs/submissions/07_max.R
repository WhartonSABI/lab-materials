...
# naive difference in means: 0.08 pts
naive_estimate <- with(
  analysis_data,
...
# overlap plot: Overlap looks strongest from around 0.1 to 0.2, with a dip in overlap around 1.75
...
# pre-adjustment balanceL Some of the covariates appear to be related to calling a timeout
# timeouts seem to occur at later quarters (smd = 0.134), and with less time remaining in the game(smd = -0.165)
...
d1 <- analysis_data %>% mutate(timeout_now = 1)
d0 <- analysis_data %>% mutate(timeout_now = 0)

mu1 <- predict(reg_model, newdata = d1)
mu0 <- predict(reg_model, newdata = d0)

reg_effect <- mean(mu1 - mu0)
reg_effect
#regression effect: 0.08 pts
...
matched_att # matched diff: 0.05 pts

balance_table( # covariates appear more balanced this time, with current margin being the largest smd
  data = matched_data,
  covariates = covariates,
  treatment = "timeout_now"
)
...
#covariates are balanced, -0.01 for period number is the largest
...
estimate_table
#models moslty agree that timeouts have a very small positive differential, with naive difference and regression adjustment providing a higher value
#Regression and IPW look at ATE, or the effect over all opportunities, while matching looks at ATT, the effect when a timeout is actually called
...
#Other confounds: tactical mismatches, fatigue, crowd effects
#Robustness Value is 0.0063
#The models are close enough to 0 with 0 being within the confidence interval, so the difference isn't that significant
#Robustness value of 0.0063 is also very small, meaning that an unobserved confounder would only need to explain 0.63% of residual to change the conclusion.
#Strongest causal claim I'd be willing to make is that timeouts have little observed effect on the score.
