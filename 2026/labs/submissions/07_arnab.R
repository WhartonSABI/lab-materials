...
#17000 decision opportunities

# timeout rate
to_rate = mean(analysis_data$timeout_now)
print(to_rate)
...
#Unit is points, treatment is timeout, control is with no timeout, outcome is a slightly positive points scored on average, in the next 3 minutes, which is also target.
#Five controls: Quarter, time remaining, pregame spread, home, current margin
...
##Overlap shows that the same pattern is mostly followed, but the ratio between expected propensity score and the actual decision taken is less.
#the smd for covariates shows that quarter and time remaining may have some differences that could affect estimation
...
#Regression adjustment is pretty close to the naive difference in terms of increase in percent margin of points in next 180 seconds. Matching and IPW are less, because they focus on similar situations.
#Covariate balance seems to have improved as they are below .01. 
##########################################
