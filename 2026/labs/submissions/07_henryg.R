...
#1. There are 17000 decision opportunities 
#2. 0.132 of the opportunities resulted in a timeout
#3. The naive difference is 0.0828
#4. The unit is a opportunity to call a timeout after a run. The treatment 
# is when a timeout is called. The control is when a timeout isn't called.
# The outcome is the change in a team's score 180 seconds after the opportunity.
# The target estimand is the average causal effect of calling a timeout among 
# the times where one was actually called. 
#5 I plan to adjust for time remaining, home vs away, size of swing, 
#score at the time, and quality of teams
...
#2. It seems like there is a higher frequency of timeouts called at higher 
#scores so it seems to make sense. 
...
#3. Before adjustment, a larger period number and current margin make calling
# a timeout more likely while the other covariates decrease the probability. 
...
#The estimates for matching ATT and IPW are visibly less than that of 
#the first two methods. This means that overall, they do not agree. This is 
#likely because the last 2 methods adjust for selection bias, which brings 
#down the estimate since coaches tend to call timeouts in useful situations
#due to variables we have not accounted for
...
#Confounding based on factors that coaches personally consider like player fatigue,
#injury risk, and ideas for specific plays they want to run make sense but have
#not been accounted for. Based on the graph, the conclusions appear to be very
#sensitive to bias. From this data, I am not willing to make any causal claim.
#What could make the design more convincing is accounting for more factors. 
